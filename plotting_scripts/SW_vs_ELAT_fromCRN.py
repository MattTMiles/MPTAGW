...
pars = list(open("/fred/oz002/users/mmiles/MPTA_GW/enterprise_newdata/out_ptmcmc/PTA_RUN/CRN_pars_noER.txt").readlines())
pars = [ p.strip("\n") for p in pars ]

result_psr = np.load("/fred/oz002/users/mmiles/MPTA_GW/enterprise_newdata/out_ptmcmc/PTA_RUN/CRN_all_updated_short.npy")
...
elat_4_idxs =  np.where((-42.5 < elats) * (elats < -27.5))[0]
elat_5_idxs =  np.where((-27.5 < elats) * (elats < -12.5))[0]
elat_6_idxs =  np.where((-12.5 < elats) * (elats < -7.5))[0]
elat_7_idxs = np.where((-7.5 < elats) * (elats < -2.5))[0]
elat_8_idxs =  np.where((-2.5 < elats) * (elats < 2.5))[0]
elat_9_idxs =  np.where((2.5 < elats) * (elats < 7.5))[0]
elat_10_idxs =  np.where((7.5 < elats) * (elats < 12.5))[0]
elat_11_idxs =  np.where((12.5 < elats) * (elats < 27.5))[0]
elat_12_idxs =  np.where((27.5 < elats) * (elats < 40))[0]
...
elats_revised.append(-35)
p5adjust = p5/(np.sum(p5)*bindiff)
elats_revised.append(-21.25)
p6adjust = p6/(np.sum(p6)*bindiff)
elats_revised.append(-11.25)
...
elats_revised.append(21.125)
p12adjust = p12/(np.sum(p12)*bindiff)
elats_revised.append(33.75)
...
axes.axvline(-15, color="grey", linestyle="-", alpha=0.25)
...
axes.axvline(0, color="gray", linestyle="--", alpha=0.5, linewidth=2)
axes.set_ylabel(r"$\mathrm{n_{\oplus}} (\mathrm{cm}^{-3})$", fontsize=14)
axes.set_xlabel(r"Ecliptic Latitude ($^{\circ}$)", fontsize=14)
axes.tick_params(axis="both", labelsize=12)
axes.set_xlim(-90, 40)
axes.set_xticks([-90, -72.5, -57.5, -42.5, -27.5, -15, -7.5, -2.5, 2.5, 7.5, 15, 27.5, 40])
