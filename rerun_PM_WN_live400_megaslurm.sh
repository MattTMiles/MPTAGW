...
if [[ ! -f "/fred/oz002/users/mmiles/MPTA_GW/enterprise/out_ppc/live_400/${psr}_PM_WN_NO_EQUAD/PM_WN_NO_EQUAD_result.json" ]] && [[ ! "${psr}_PM_WN_NO_EQUAD" == $(squeue --format="%.18i %.9P %.50j %.8u %.8T %.10M %.9l %.6D %R" --me | awk '{print $3}' | grep -w ^${psr}_PM_WN_NO_EQUAD) ]]; then
    sbatch -J ${psr}_PM_WN_NO_EQUAD /home/mmiles/soft/GW/PM_WN_noise_mpi_slurm.sh ${psr} PM_WN_NO_EQUAD "PM_WN_NO_EQUAD" 400
    echo "rerunning ${psr}_PM_WN_NO_EQUAD"
fi
