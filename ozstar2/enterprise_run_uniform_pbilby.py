...
parser.add_argument("-noise_search", type = str.lower, nargs="+",dest="noise_search", help="The noise parameters to search over. Timing model is default. Include as '-noise_search noise1 noise2 noise3' etc. The _c variations of the noise redirects the noise to the constant noisefile values", \
    choices={"efac", "equad", "t2_equad", "ecorr", "red", "efac_c", "equad_c", "ecorr_c", "ecorr_check", "red_c", "dm", "chrom", "chrom_c","chrom_cidx","high_comp_chrom", "dm_c", "gw", "gw_const_gamma","gw_const_gamma_wide", "lin_exp_gw_const_gamma_wide", "gw_c", "dm_wide", "dm_wider", "red_wide", "chrom_wide", "chrom_cidx_wide", "efac_wide",\
        "band_low","band_low_c","band_high","band_high_c", "band_high_wide", "spgw", "spgwc", "spgwc_wide", "spgwc_18", "pm_wn", "pm_wn_no_equad", "pm_wn_sw","pm_wn_altpar", "pm_wn_no_equad_altpar", "wn_sw", "wn_tester", "chrom_annual", "sw", "swdet", "free_spgw", "free_spgwc", "hfred", "pm_wn_hc", "pm_wn_sw_hc", "pm_wn_sw_hc_noeq","pm_wn_hc_noeq",\
            "hc_chrom_cidx", "hc_red", "hc_dm", "hc_chrom", "mpta_pm", "mpta_pm_gauss_ecorr", "extra_red", "smbhb", "smbhb_const", "ecorr_gauss", "ecorr_gauss_c", "smbhb_wn", "extra_chrom_annual", "dm_gauss_bump", "chrom_gauss_bump", "extra_chrom_gauss_bump"})
...
def prior_transform_function(theta):
  ...
  for i in range(len(pta.params)):
    ...
    elif pta.params[i].type == "linearexp":
        #cube[i] = ( pta.params[i].prior.func_kwargs['pmax'] - pta.params[i].prior.func_kwargs['pmin'])*theta[i] + pta.params[i].prior.func_kwargs['pmin']
        #np.log10(val*(10**self.maximum-10**self.minimum) + 10**self.minimum)
        cube[i] = np.log10(theta[i]*(10**pta.params[i].prior.func_kwargs["pmax"] - 10**pta.params[i].prior.func_kwargs["pmin"]) +10**pta.params[i].prior.func_kwargs["pmin"]) 
  return list(cube)
...
def dm_gaussian_bump(tmin, tmax, idx=2, sigma_min=20, sigma_max=1000,
    ...
    log10_Amp_dm_bump =  parameter.LinearExp(log10_A_low, log10_A_high)
    if idx == 'vary':
...
for n in noise:
    ...
    if "equad" == n:
        equad =  parameter.LinearExp(-10,-1) 
    if "t2_equad" == n:
        t2_equad =  parameter.LinearExp(-10,-1) 
    if "equad_c" == n:
        equad = parameter.Constant()
    if "ecorr" == n:
        ecorr =  parameter.LinearExp(-10,-1) 
    if "ecorr_gauss" == n:
        ecorr =  parameter.LinearExp(-10,-1)
    if "ecorr_gauss_c" == n:
    ...
    if "red" == n:
        ...
        log10_A_red =  parameter.LinearExp(-20, -11)
    ...
    if "hfred" == n:
        log10_A_hfred =  parameter.LinearExp(-20, -11)
    ...
    if "chrom" == n:
        log10_A_chrom_prior =  parameter.LinearExp(-20, -11)
    ...
    if "band_low"== n:
        log10_A_bn =  parameter.LinearExp(-20, -11)
...
for n in noise:
    ...
    if "chrom" == n or "chrom_wide" == n or "chrom_c" == n:
        chrom_model = utils.powerlaw(log10_A=int(log10_A_chrom_prior),
    ...
    if "dm_gauss_bump" == n:
        gauss_bump = dm_gaussian_bump(tmin, tmax, idx=2, sigma_min=20, sigma_max=1000, log10_A_low=-10, log10_A_high=-1, name='dm_bump')
        s += gauss_bump

    if "chrom_gauss_bump" == n:
        gauss_bump = dm_gaussian_bump(tmin, tmax, idx="vary", sigma_min=20, sigma_max=1000, log10_A_low=-10, log10_A_high=-1, name='chrom_bump')
        s += gauss_bump
    ...
    if "sw" == n:
        ...
        sw_components = 120
        log10_A_sw =  parameter.LinearExp(-10, 1)
    ...
    if "spgw" == n or "spgwc" == n or "spgwcm" == n or "spgwc_18" == n or "spgwc_wide" == n:
        ...
        #This is a place holder for a quick monopole check, this is not finished, activating it won't do anything
            ...
            log10_A_gw =  parameter.LinearExp(-18,-17.8)('log10_A_gw')
            gamma_gw = parameter.Constant(4.33)('gamma_gw')
        else:
            log10_A_gw =  parameter.LinearExp(-18,-12)('log10_A_gw')
            gamma_gw = parameter.Uniform(0,7)('gamma_gw')
        ...
        for i, n in enumerate(noise):
            if "extra_red" == n:
                if len([ pn for pn in psrmodels if "RN" in pn ]) == 0:
                    log10_A_red =  parameter.LinearExp(-20, -12)
                    gamma_red = parameter.Uniform(0, 7)
                    pl = utils.powerlaw(log10_A=log10_A_red, gamma=gamma_red)
                    rn = gp_signals.FourierBasisGP(spectrum=pl, components=high_comps, Tspan=Tspan)
                    s += rn
    ...
    if "smbhb" == n or "smbhb_const" == n or "smbhb_wn" == n:
        ...
        wn_json = json.load(open("/fred/oz002/users/mmiles/MPTA_GW/SMBHB/new_models/august23data/MPTA_WN_models.json"))
        wnkeys = list(wn_json.keys())
        wnmodels = [ wn_model.split("_")[-1] for wn_model in wnkeys if pulsar in wn_model ]
        for i, pm in enumerate(psrmodels):
        ...
        for i, n in enumerate(noise):
            if "extra_chrom_gauss_bump" == n:
                if len([ pn for pn in psrmodels if "CHROMBUMP" in pn ]) == 0:
                    chrom_gauss_bump = dm_gaussian_bump(tmin, tmax, idx="vary", sigma_min=20, sigma_max=2000, log10_A_low=-10, log10_A_high=-1, name='chrom_bump')
                    s += chrom_gauss_bump
