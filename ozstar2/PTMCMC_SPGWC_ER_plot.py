...
def chain_corner_from_dir(pulsar, parstxt):
    ...
    chainfiles = glob.glob("/fred/oz002/users/mmiles/MPTA_GW/enterprise_ozstar2/out_ptmcmc/SPGWC/" + pulsar + '/SPGWC_ER*/chain_1.txt')

    chains = []

    for chainfile in chainfiles:
        if "WIDE" not in chainfile:
    ...
    figsavename = pulsar_dir + '/{}_ER_corner'.format(pulsar) + \
        corner_file_label + '.png'
    plt.savefig(figsavename, dpi=300, bbox_inches='tight')
    plt.close()
    print("Made: "+figsavename)

    np.save(pulsar_dir+"/"+pulsar+"_ER_burnt_chain", chain)
