    ...
    for i in tqdm(range(N), position=0, leave=True):
        ...
        if len(weights) > 4:
            if len(weights) % 50 == 0:
                ...
                plt.hist(p_CRN_gwamp, bins=50, histtype="stepfilled",label="CRN",alpha=0.25,color="C0",lw=2, density=True)
                plt.hist(p_CRN_gwamp, weights=weights_save, bins=50, histtype="stepfilled",label="HD reweighted",alpha=0.25,color="C1",lw=2, density=True)
                plt.legend()
            ...
            if len(weights) % 100 == 0:
                ...
                p_HD_gwgamma = np.random.choice(p_CRN_gwgamma,size=len(weights), p=temp_weights/np.sum(temp_weights))
                p_HD_gwamp = np.random.choice(p_CRN_gwamp,size=len(weights), p=temp_weights/np.sum(temp_weights))
...
p_HD_gwgamma = np.random.choice(p_CRN_gwgamma,size=len(weights), p=weights/np.sum(weights))
