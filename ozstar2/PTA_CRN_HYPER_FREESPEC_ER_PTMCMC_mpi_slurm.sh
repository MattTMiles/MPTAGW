...
touch "PTA_CRN_PL_ER_HYPER_VARYGAMMA_PTMCMC_${1}"

echo "PTA_CRN_PL_ER_HYPER_VARYGAMMA_PTMCMC_${1}"
ulimit -s 16384
...
while [ $i -lt 1 ]; do
    ...
    python /home/mmiles/soft/GW/ozstar2/CRN_PTMCMC_HYPERMODEL.py -pta /fred/oz002/users/mmiles/MPTA_GW/enterprise_ozstar2/out_ptmcmc/pta_objects/PTA_FREESPEC_CRN_ER_FULL.pkl -results FREESPEC_ER_run_$1 -alt_dir out_ptmcmc/PTA_RUN/HYPER_FREESPEC/
    if [[ "$?" -eq 139 ]]; then
        echo "segfault !";
...
rm -f "PTA_CRN_PL_ER_HYPER_VARYGAMMA_PTMCMC_${1}"
