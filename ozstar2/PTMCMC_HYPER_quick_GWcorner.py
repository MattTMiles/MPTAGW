...
def gw_corner_all():
    ...
    chain_all = []
    burnt_chain_all = []
    for chaindir in glob.glob("/fred/oz002/users/mmiles/MPTA_GW/enterprise_ozstar2/out_ptmcmc/PTA_RUN/HYPER/CRN_ER_run*"):
            ...
            try:
                ...
                if lenchain > 20000:
                    ...
                    if not likelihood == "-inf":
                        ...
                        burnt_chain = burnt_chain[::10,:]
                        chain_gamma = burnt_chain[::10,-7]
                        chain_amp = burnt_chain[::10,-6]
                        chain_gw = np.vstack([chain_gamma,chain_amp])
                        #chain_gws.append(chain_gw)
                        #np.save(chaindir+"/burnt_chain_all", burnt_chain)
    ...
    fig = corner.corner(chaingw.T,bins=30,labels=["$\gamma_\mathrm{CRN}$","$\log_{10}\mathrm{A_{CRN}}$"],quantiles=(0.16,0.84), show_titles=True,title_quantiles=[0.16,0.5,0.84], smooth=False, smooth1d=False, plot_contours=True, plot_datapoints=False, plot_density=True, levels=[0.393, 0.864, 0.989], fill_contours=False, color="teal", label_kwargs={"fontsize":14}, figsize=(19.20,10.80))
    ...
    fig.savefig("/fred/oz002/users/mmiles/MPTA_GW/enterprise_ozstar2/out_ptmcmc/PTA_RUN/HYPER/CRN_PL_ER_GW_trimmed_corner_3sigma_noKDE.png")
    fig.clf()

    np.save("/fred/oz002/users/mmiles/MPTA_GW/enterprise_ozstar2/out_ptmcmc/PTA_RUN/HYPER/CRN_ER_GW_updated",chaingw)
    #np.save("/fred/oz002/users/mmiles/MPTA_GW/enterprise_ozstar2/out_ptmcmc/PTA_RUN/HYPER/CRN_ER_all_updated",chainall)
    #p.save("/fred/oz002/users/mmiles/MPTA_GW/enterprise_ozstar2/out_ptmcmc/PTA_RUN/HYPER/CRN_ER_burntall_updated",burntchainall)
