...
parser.add_argument("-type", dest="type", choices={"full", "white", "red", "inc_ER"}, help="The type of nosise to extract", required=True)
...
if extract == "white":
    ...
    chains = sorted(glob.glob(post_dir+"/J*/thinned_chain_combined.npy"))
    ...
    for chain in chains:
        chaindir = chain.rstrip("thinned_chain_combined.npy")
        chain_i = np.load(chain).squeeze()
        #temp_df = pd.read_json(json.load(open(file)))
        #psrname = temp_df.columns[0].split("_")[0]
        psrname = chain.split("/")[-2]
