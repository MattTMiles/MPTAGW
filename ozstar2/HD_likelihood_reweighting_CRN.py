...
outdir = "/fred/oz002/users/mmiles/MPTA_GW/enterprise_newdata/out_ptmcmc/HD_reweighting/CRN_ER/HD_RW_{0}".format(output_num)
...
pta_CRN_file = "/fred/oz002/users/mmiles/MPTA_GW/enterprise_newdata/out_ptmcmc/pta_objects/MPTA_CRN_ER.pkl"
pta_HD_file = "/fred/oz002/users/mmiles/MPTA_GW/enterprise_newdata/out_ptmcmc/pta_objects/MPTA_HD_ER.pkl"

#pta_CRN = dill.load(open(pta_CRN_file,"rb"))
pta_HD = dill.load(open(pta_HD_file,"rb"))

posterior_CRN_file = "/fred/oz002/users/mmiles/MPTA_GW/enterprise_newdata/out_ptmcmc/PTA_RUN/CRN_ER_all_updated_short.npy"
...
try:
    ...
    temp_weights = list(np.load(outdir+"/weights.npy"))
except:
    weights = []
    temp_weights = []
    pass
    ...
    for i in tqdm(range(N), position=0, leave=True):
        ...
        if not Ns in Ns_all:
            ...
            if weight_i > 1e-100:
                ...
                temp_weights.append(weight_i)
                mean_weight = np.mean(temp_weights)

                p_HD_gwamp = np.log10((weight_i/mean_weight)*(10**posterior_CRN[Ns,-6].T))
                p_HD_gwgamma = (weight_i/mean_weight)*(posterior_CRN[Ns,-7].T)

                if (p_HD_gwamp >= -18) * (p_HD_gwamp <= -11) * (p_HD_gwgamma >= 0) * (p_HD_gwgamma <= 7):
        ...
        if len(weights) > 10:
            if len(weights) % 10 == 0:
                ...
                p_HD_gwgamma = (weights/mean_weight)*(partial_CRN_post_save[:,-7].T)
                
                phys_check = (p_HD_gwamp >= -18) * (p_HD_gwamp <= -11) * (p_HD_gwgamma >= 0) * (p_HD_gwgamma <= 7)
                p_HD_gwamp = p_HD_gwamp[phys_check]
                p_HD_gwgamma = p_HD_gwgamma[phys_check]

                posterior_HD_save = np.vstack([p_HD_gwgamma,p_HD_gwamp])
                np.save(outdir+"/posterior_HD_RW",posterior_HD_save)

                partial_CRN_post_save = partial_CRN_post_save[phys_check]
                np.save(outdir+"/partial_CRN_post",partial_CRN_post_save)
                
                weights_save = weights_save[phys_check]
