...
cross_corr_dir = "/fred/oz002/users/mmiles/MPTA_GW/enterprise_newdata/PAIRWISE_433/"
...
for pair in pp_list:
    ...
    if "J0000000" not in pairname:
        ...
        try:
            lenchain = os.popen("cat "+cross_corr_dir + "/" + pairname + "/master_chain_new.txt | wc -l").read().strip("\n")
            burn = int(0.5*int(lenchain))
            res_pair = np.loadtxt(cross_corr_dir + "/" + pairname + "/master_chain_new.txt", skiprows=burn)
        except:
            lenchain = os.popen("cat "+cross_corr_dir + "/" + pairname + "/master_chain_new.txt | wc -l").read().strip("\n")
            burn = int(0.5*int(lenchain))
            trialpd = pd.read_csv(cross_corr_dir + "/" + pairname + "/master_chain_new.txt", sep="\t", header=None, on_bad_lines="skip", skiprows=burn+1)
        ...
        if len(res_pair) > 500:
            ...
            np.save("/fred/oz002/users/mmiles/MPTA_GW/PAIRWISE_NEW/thinned_chains_corrAmp_433/"+pairname, res_pair)
